...
WIDTH = 1500
HEIGHT = 900
FPS = 50 #high Frame Rate is mainly due to the mouse, so movement can remain smooth, it does not affect the game too much.
...
##Animations
Background = [pygame.image.load("level 1.png"),pygame.image.load("level 2.png")]
bg = pygame.image.load("newbg2.png")
...
##File Searching
Highscore_file = open("Highscr.txt","r")
Highscore = Highscore_file.read().strip()
...
class Seeker(pygame.sprite.Sprite):
        ...
        def update(self):
            ...
            if self.rect.x <= -100:
                ...
                self.speedx = random.randint(-20,-10)
                self.right = False
                self.rect.x += self.speedx
                self.rect.centery = player.rect.centery
...
class Pong(pygame.sprite.Sprite):
        ...
        def __init__(self):
            ...
            self.speedy = random.randint(7,12)
            self.speedx = random.randint(7,12)
            self.bounced = 0
...
while running:
    ...
    HighscoreMessage = FONT.render("Highscore:"+str(Highscore),False,WHITE)


    if Score*10 > Highscore and Admincommands == False:
        Highscore_file = open("Highscr.txt", "w")
        Highscore_file.write(str(Score*10))
        Highscore_file.close()
        Highscore = Score*10
...
##    screen.fill(BLACK)
    ...
    if GameSTART == True and Gameover == False:
        ...
        screen.blit(ScoreMessage,(WIDTH-300,30))
        screen.blit(HighscoreMessage,(WIDTH-300,70))
